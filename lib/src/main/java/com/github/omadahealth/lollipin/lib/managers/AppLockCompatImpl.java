...
public class AppLockCompatImpl<T extends AppLockCompatActivity> extends AppLock implements LifeCycleInterface {

    public static final String TAG = "AppLockCompatImpl";
    ...
    /**
     * The {@link SharedPreferences} key used to store whether the user has backed out of the {@link AppLockCompatActivity}
     */
    ...
    /**
     * The activity class that extends {@link AppLockCompatActivity}
     */
    private Class<T> mActivityClass;

    /**
     * Static instance of {@link AppLockCompatImpl}
     */
    private static AppLockCompatImpl mInstance;

    /**
     * Static method that allows to get back the current static Instance of {@link AppLockCompatImpl}
     *
     * @param context       The current context of the {@link Activity}
     * @param activityClass The activity extending {@link AppLockCompatActivity}
     * @return The instance.
     */
    public static AppLockCompatImpl getInstance(Context context, Class<? extends AppLockCompatActivity> activityClass) {
        synchronized (LockManager.class) {
            if (mInstance == null) {
                mInstance = new AppLockCompatImpl<>(context, activityClass);
            }
        }
        return mInstance;
    }

    private AppLockCompatImpl(Context context, Class<T> activityClass) {
        super();
    ...
    public boolean shouldLockSceen(Activity activity) {
        ...
        // already unlock
        if (activity instanceof AppLockCompatActivity) {
            AppLockCompatActivity ala = (AppLockCompatActivity) activity;
    ...
    public void onActivityPaused(Activity activity) {
        ...
        if (!shouldLockSceen(activity) && !(activity instanceof AppLockCompatActivity)) {
            setLastActiveMillis();
        }
    }
    ...
    public void onActivityUserInteraction(Activity activity) {
        if (onlyBackgroundTimeout() && !shouldLockSceen(activity) && !(activity instanceof AppLockCompatActivity)) {
            setLastActiveMillis();
        }
    }
